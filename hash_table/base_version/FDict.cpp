 ...
 *
 /--------------------------------------------------------*/
//TODO second candidate to been refactored with asm
 ...
 *
 /-------------------------------------------------------------------------------------------*/
inline Node* Node::fins(const char *word)
{
    ...
    THROW(UNKNOWN_ERROR, "Incorect cases in Node::find_and_insert", nullptr);
}
...
{
    return table_[hash_func_(word) % oak::DICT_SIZE].find(word);
}
...
Node* Dictionary::fins(char* word)
{
    ...
    Node* current_node = table_[hash_func_(word) % oak::DICT_SIZE].fins(word);
