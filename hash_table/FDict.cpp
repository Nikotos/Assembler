 ...
 *
 /==============================================================================*/
#define HASH_FUNCTION(DATA) GNU_HASH_ASM(DATA)
...
oak::Word::Word(const char *name)
{
    ...
    {
        FTHROW(LOGIC_ERROR, "Word is too big!");
    }
    else
    {
        body_ = new char[len];
 ...
 *
 /--------------------------------------------------------*/
//TODO запилить вызов new с инициализатором - done
 ...
 *
 /--------------------------------------------------------*/
//TODO second candidate to been refactored with asm - hard to do and will be useful only in the right use-case
 ...
 *
 /-------------------------------------------------------------------------------------------*/
Node* Node::find(const char *word)
{
    if ((word == nullptr) || (*word == '/0'))
    {
        THROW(LOGIC_ERROR, "Incorrect word-input in Node::find()", nullptr);
    }

    /* little optimisation for short lists*/
    if (THE_FASTEST_WORDCMP(word, word_.body_) == 0)
    {
        return this;
    }

    Node* current_node = next_node_;
 ...
 *
 /-------------------------------------------------------------------------------------------*/
Node* Node::old_fins(const char* word)
{
    /* creating new table-in Node */
    if (frequency_ == 0)
    {
        re_set(word);
        return this;
    ...
    while (current_node != nullptr)
    {
        if (THE_FASTEST_WORDCMP(current_node->word_.body_, word) == 0)
        {
            return current_node;
        }
...
/*-------------------------------------------------------------------------------------------
 *                          THE MAIN OPTIMIZATION HERE! BOSS!!
 *
 ...
 *
 /-------------------------------------------------------------------------------------------*/
//TODO truly-second candidate to been refactored with __asm__() - done
Node* Node::fins(const char *word)
{
    ...
    else if(THE_FASTEST_WORDCMP(word_.body_, word) == 0)
    {
        return this;
    }
    else if (next_node_ == nullptr)
    {
        next_node_ = new Node(word);
        return next_node_;
    }
    ...
    else if (ret_node->next_node_ == nullptr)
    {
        ret_node->next_node_ = new Node(word);
        return ret_node->next_node_;
    }

    /* return nullptr when some gone wrong*/
    return nullptr;
}
...
#define CHECK_HASH_FUNCTION\
    ...
    {\
        THROW(LOGIC_ERROR, "You haven't chosen hash function!", nullptr);\
    }\
...
{
    return table_[HASH_FUNCTION(word) % oak::DICT_SIZE].find(word);
}
...
Node* Dictionary::fins(char* word)
{
    ...
    Node* current_node = table_[HASH_FUNCTION(word) % oak::DICT_SIZE].old_fins(word);
...
void Dictionary::pars_file(const char *file_name)
{
    ...
    {
        fins(dealer.text_[i].body_)->frequency_++;
    }
